...
public class SAL2115DAO : BaseDAO
{
        ...
        )
    {
        ...
        if(strUnit != "ALL")
        {
            strSQL += "  AND B.BASE_DEP IN (@strUnit)";
        }
        if (strPayBudgeCode != "" && strPayBudgeCode != "ALL")
        {
            strSQL +=
            " and I.INCO_Budget_code = @strPayBudgeCode "; //查詢畫面選擇之預算來源代碼
        }
        
        strSQL += "  ORDER BY BASE_IDNO ";
        ...
        SqlParameter[] sp =
        {
            ...
            new SqlParameter("@strPayBudgeCode",strPayBudgeCode) //預算來源        
        };
        ...
        )
    {
        ...
        strSQL += "  AND I.INCO_ORGID = @LogDeptCD "
              + "  AND ("
              + "  (I.INCO_CODE IN ('001', '007') AND I.INCO_YM LIKE @YearMonth + '%')"
              + "  OR (I.INCO_CODE IN ('002', '003', '004','005', '006') AND I.INCO_DATE LIKE RTRIM(@YearMonth) + '%')"
              + "  ) "
