...
/// 晉級補發發放清冊
/// </summary>
public class SAL2109DAO : BaseDAO
{
    public SAL2109DAO()
    {
        //
    ...
    public SAL2109DAO(SqlConnection conn)
        : base(conn)
    {
    ...
    )
    {
        string strSQL =
            ...
            ", isnull(( select sum(PAYOD_AMT) from SAL_SAPAYOD where PAYOD_ORGID = PAYO_ORGID and PAYOD_SEQNO = PAYO_SEQNO and PAYOD_KIND = PAYO_KIND and PAYOD_YM = PAYO_YYMM and PAYOD_DATE = PAYO_DATE and PAYOD_CODE_SYS = '003' and PAYOD_CODE_TYPE = '002' and PAYOD_CODE_NO = '002' and PAYOD_CODE = '001'),0) as item011 " + //公健保費
            ", isnull(( select sum(PAYOD_AMT) from SAL_SAPAYOD where PAYOD_ORGID = PAYO_ORGID and PAYOD_SEQNO = PAYO_SEQNO and PAYOD_KIND = PAYO_KIND and PAYOD_YM = PAYO_YYMM and PAYOD_DATE = PAYO_DATE and PAYOD_CODE_SYS = '003' and PAYOD_CODE_TYPE = '002' and PAYOD_CODE_NO = '018' and PAYOD_CODE = '001'),0) as item012 " + //眷屬健保費
            ...
            ", isnull(( select sum(PAYOD_AMT) from SAL_SAPAYOD where PAYOD_ORGID = PAYO_ORGID and PAYOD_SEQNO = PAYO_SEQNO and PAYOD_KIND = PAYO_KIND and PAYOD_YM = PAYO_YYMM and PAYOD_DATE = PAYO_DATE and PAYOD_CODE_SYS = '003' and PAYOD_CODE_TYPE = '002' and PAYOD_CODE_NO = '002'),0) as item015  " + //勞健保費
            ", isnull(( select sum(PAYOD_AMT) from SAL_SAPAYOD where PAYOD_ORGID = PAYO_ORGID and PAYOD_SEQNO = PAYO_SEQNO and PAYOD_KIND = PAYO_KIND and PAYOD_YM = PAYO_YYMM and PAYOD_DATE = PAYO_DATE and PAYOD_CODE_SYS = '003' and PAYOD_CODE_TYPE = '002' and PAYOD_CODE_NO = '018'),0) as item016  " + //眷屬勞健保費
            ...
            "and PAYO_KIND =  '006'    "; //固定為006 晉級補發發放清冊
        if (strPayoProno == "2")
        {
            strSQL +=
                "and PAYO_PRONO <> '007'  "; //-- 若人員類別選擇   全部(不含臨時工)，增加此查詢條件
        }
        if (strPayoProno == "3")
        {
            strSQL +=
                "and PAYO_PRONO = '007'  "; //-- 若人員類別選擇   臨時工，增加此查詢條件
        }
