...
Namespace FSC.Logic
    Public Class FSC3110DAO
        ...
        Public Function getQueryData(ByVal Orgcode As String, _
            ...
            sql.AppendLine(" m.Orgcode, p.User_name, p.Id_card, ")
            sql.AppendLine(" f.flow_id, m.Start_date, m.End_date, m.Start_time, m.End_time, m.reason, f.case_status, ")
            sql.AppendLine(" case when m.leave_ngroup='E1' then '一般加班' when m.Leave_ngroup='E2' then '專案加班' when m.Leave_ngroup ='E3' then '大批加班'  end as casetype, ")
            sql.AppendLine(" case when m.Leave_ngroup ='E3' then 'True' else 'False' end as del_mode ")
            ...
            sql.AppendLine(" where m.orgcode=@orgcode and m.leave_group='E' ")
            ...
            If Not String.IsNullOrEmpty(case_status) Then
                sql.AppendLine(" and f.case_status = @case_status and f.last_pass = '1' ")
            End If
            If Not String.IsNullOrEmpty(praytype) Then
                sql.AppendLine(" and m.Leave_ngroup = @praytype ")
            End If
            ...
            aryParms(6) = New SqlParameter("@praytype", SqlDbType.VarChar)
            aryParms(6).Value = "E" + praytype
