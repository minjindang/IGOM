...
public partial class SAL_SAL3_SAL3107_01 : BaseWebForm
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        UcDDLDepart1.Orgcode = strOrgCode;
...
//        pnlInput.Visible = false;
    ...
    private void getData()
    {
        ...
        DataTable dt = sal3107.queryUser(v_UserOrgId, v_bouns_year, v_base_job, v_base_status, v_Search_IDCard, v_Search_Name, strDepart);
    ...
    protected void Button_sabase_Click(object sender, EventArgs e)
    {
        // 參數
        string v_UserOrgId = this.strOrgCode;    // 單位
        string v_bouns_year = Convert.ToString(Convert.ToInt32(cmbYear.Year.ToString())+1911);
        SAL3107 sal3107 = new SAL3107();

        // chaeck data
        if (sal3107.CheakSalSaseBouns4Insert(v_UserOrgId,v_bouns_year))
        {
            sal3107.insertBouns(v_UserOrgId, v_bouns_year, strLogIDCard);
            getData();
        }
        else
        {
            ...
            CommonFun.MsgShow(ref tempPage,
               CommonFun.Msg.Custom, "已經有已帶入的主管加給資料不可重覆帶入", "", "");
            this.Page = tempPage;
        }
    ...
    protected void gvResult_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            gvddlBossAdd.Rebind();
            Label KDC = (Label)e.Row.FindControl("kdc");
            gvddlBossAdd.SelectedValue = KDC.Text.Trim();
...
//            {
                ...
                string BOUNS_KDC = ((uc_ucSaCode)info_gvr.FindControl("cmbBossAdd")).Code_no;
                string BOUNS_KDC_SERIES = ((uc_ucSaSpesup)info_gvr.FindControl("UcSaSpesup1")).v_Series;
                string BOUNS_KDC_MON = ((DropDownList)info_gvr.FindControl("ddl_month")).SelectedValue;
                string BOUNS_KDC_AMT = ((TextBox)info_gvr.FindControl("txt_KDC_AMT")).Text;

                string vv_id = B_ID.Text.Trim();
                string vv_year = BOUNS_YEAR.Text.Trim();
                string vv_kdc = BOUNS_KDC.Trim();
                string vv_kdc_series = BOUNS_KDC_SERIES.Trim();
                string vv_kdc_mon = BOUNS_KDC_MON.Trim();
                string vv_muser = this.strLogIDCard;


                emp3107.updateSalSaseBouns(vv_id, vv_year, vv_kdc, vv_kdc_series, vv_kdc_mon, vv_muser, BOUNS_KDC_AMT.Trim());
//            }
...
//        Response.Write("UcSaCode1_CodeChanged<BR>");
    ...
    protected void btn_Delete_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow info_gvr in this.gvResult.Rows)
        {
            ...
            if (CheckBox_Seqno)
            {
                ...
                emp3107.deleteSalSaseBouns(vv_id, vv_year);
            }
        }
        this.getData();
    ...
    protected void CheckBox_Seqno_CheckedChanged1(object sender, EventArgs e)
    {
...
//                this.div_search.Style.Item("display") = "none";
    ...
    protected void Button_Commit_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow gvr in GridView_Base.Rows)
        {
            ...
            {
                SAL3107 sal3107 = new SAL3107();
                if (sal3107.CheakSalSaseBouns4Insert(v_UserOrgId, v_bouns_year, v_seqno))
                {
                    //Response.Write("DO insert : " + v_seqno);
                    sal3107.insertBounswithBaseSeqNO(v_UserOrgId, v_bouns_year, strLogIDCard, v_seqno);
                }
