...
public partial class SAL3122_03 : BaseWebForm
{
        ...
        if (check_data_upemp()) 
        {
            ...
            for (int i = 0; i < GridView_SaUpemp.Rows.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    if (TextBox_tabid.Text == "B2005")
                    {
                        DropDownList kind = (DropDownList)GridView_SaUpemp.Rows[i].Cells[5].FindControl("DropDownList_Kind");
                        t_kind = kind.SelectedValue;
                    }
                    else
                    {
                    ...
                    TextBox amt = (TextBox)GridView_SaUpemp.Rows[i].Cells[6].FindControl("TextBox_amt");
                    t_amt = amt.Text;

                    if (TextBox_tabid.Text == "B1224")
                    {
                        TextBox qty = (TextBox)GridView_SaUpemp.Rows[i].Cells[7].FindControl("TextBox_qty");
                        t_qty = qty.Text;
                    }
