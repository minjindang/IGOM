...
#Region "下拉式選單"
    ...
    Protected Sub CODE_TYPE_Bind()
        ddlCODE_TYPE.Items.Insert(0, New ListItem("第二層代碼", "**"))
...
#Region "連繫資料"
    Protected Sub Bind()
        ...
        Dim CODE_SYS As String = Request.QueryString("sys")
        Dim sc As SYS.Logic.CODE = New SYS.Logic.CODE
        Dim dt As DataTable = sc.GetData(Orgcode, CODE_SYS, "**", "")

        For Each dr As DataRow In dt.Rows
            dr("CODE_TYPE_Name") = "第二層代碼"
        Next
...
#Region "刪除"
    Protected Sub cbDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim dt As DataTable = sc.GetData(Orgcode, CODE_SYS, CODE_NO, "")
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
...
#End Region
    ...
    Protected Sub cbNext_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Response.Redirect("SYS3103_03.aspx?sys=" + CODE_SYS + "&kind=" + CODE_KIND + "&type=" + CODE_NO)
    End Sub
    ...
    Protected Sub cbBack_Click(sender As Object, e As EventArgs) Handles cbBack.Click
        Response.Redirect("SYS3103_01.aspx")
    End Sub
